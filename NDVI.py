...
corner_grid=input("enter OS 1km four figure corner grid reference e.g. 44000 10000").split()     #input of corner grid, split into list
...
p2= distance(kilometers=1).destination((p1), bearing=0)  #set next corner location 1km along
p3 = distance(kilometers=1).destination((p2), bearing=90)   #set next corner location 1km along
p4 = distance(kilometers=1).destination((p3), bearing=180)   #set next corner location 1km along
p0 = p1     #set a p0 to close the polygon with the same coordinates as the start point
points = [(p.longitude, p.latitude) for p in [p1,p2,p3,p4, p0]]  #convert points to lat long explicit to create polygon
km_square = Polygon(points)                         #create polygon using the 4 corner points



#search by polygon, time, and SciHub query keywords
footprint = geojson_to_wkt(read_geojson(km_square))
